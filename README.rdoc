...
Let's look at the controller specs.
  ...
  describe FoosController do
    def valid_attributes(args = {})
      {
        # Add valid attributes for the your params[:foo] here!
      }.merge(args)
    end
    ...
    describe "POST :create" do
      describe "when successful" do
        ...
        it_should_initialize :foo
        it_should_save :foo
        it_should_redirect_to { foo_url(@foo) }
      end
...
Now let's move to the view specs!
  ...
  describe "/foos/form.html.haml" do
    before(:each) do
      @foo = mock_and_assign(Foo, :stub => {
        :name => "foo",
        :birthday => 1.week.ago,
        :adult => false
      })
    ...
    it "should use form_for to generate the proper form action and options" do
      template.should_receive(:form_for).with(@foo)
      do_render
    end

    it_should_allow_editing :foo, :name
    it_should_allow_editing :foo, :birthday
    it_should_allow_editing :foo, :adult

    it_should_link_to_show :foo
    it_should_link_to { foos_path }
  end
...
There are also some really nice helper methods that I'd like point out. One is <tt>it_should_link_to_<i>controller_method</i></tt>. These methods (there's one each for
