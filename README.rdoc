...
To benefit from Skinny Spec's scaffolding, you'll need to be using
make_resourceful[http://github.com/rsl/make_resourceful/tree/master] and
Haml[http://github.com/nex3/haml/tree/master]
as well. I've got plans to add more generic Rails' scaffolding but I have <i>lots</i> of plans so
...
A useful helper method that doesn't appear in any of the scaffolding is <tt>with_default_restful_methods</tt>
which takes a block and evaluates it for each of the RESTful controller methods. Very useful for
spec'ing that these methods redirect to the login page when the user isn't logged in, for example.
